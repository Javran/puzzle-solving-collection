...
mergeSort :: forall a s. Ord a => V.Vector a -> WriterT (Sum Int) (ST s) (V.Vector a)
mergeSort xs = do
...
testMergeSort = do
  let src :: [Int]
      ...
      xs = V.fromList src
      (ys, count) = runST $ runWriterT $ mergeSort xs
  ...
  let v = V.fromList @Int [12, 1, 10, 2, 7, 11, 4, 14, 5, 9, 15, 8, 13, 6, 3]
      (_, count') = runST $ runWriterT $ mergeSort v
  print (count' == 49)
